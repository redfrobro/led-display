...
def plasma_effect(duration=8, frequency=5):
...
def fire_effect(duration=8, frequency=5):
...
def matrix_rain(duration=8, frequency=5):
    ...
    for x in range(COLS):
        if randrange(init_rate) == 0:
            drops.append({'x': x, 'y': randrange(-ROWS, 0), 'speed': randrange(1, 4), 'length': randrange(5, 15)})
    ...
    while time.time() - start_time < duration:
        ...
        if randrange(spawn_rate) == 0 and len(drops) < COLS:
            drops.append({'x': randrange(COLS), 'y': randrange(-10, 0), 'speed': randrange(1, 4), 'length': randrange(5, 15)})

        time.sleep(0.05)


def sparkle_twinkle(duration=8, frequency=5):
    ...
    while time.time() - start_time < duration:
        ...
        for _ in range(stars_per_frame):
            ...
            hue = randrange(360)
            stars[(x, y)] = {'brightness': 0, 'direction': 1, 'hue': hue, 'max': randrange(150, 255)}
        ...
        for pos, star in stars.items():
            ...
            r, g, b = hsv_to_rgb(star['hue'], 0.3, brightness / 255)
...
def meteor_shower(duration=8, frequency=5):
...
def spiral_effect(duration=8, frequency=5):
    ...
    while time.time() - start_time < duration:
        for y in range(ROWS):
            for x in range(COLS):
                hue = (ANGLE_TABLE[y][x] + int(DIST_TABLE[y][x] * 10) + hue_offset) % 360
                r, g, b = hsv_to_rgb(hue, 1.0, 1.0)
                matrix.SetPixel(x, y, r, g, b)
...
def bouncing_balls(duration=8, frequency=5):
...
def lightning(duration=8, frequency=5):
    ...
    while time.time() - start_time < duration:
        ...
        if randrange(int(spawn_rate)) == 0:
            ...
            bolts.append({'points': points, 'life': max_life, 'max_life': max_life})

        time.sleep(0.04)


def fireworks(duration=8, frequency=5):
...
def starfield(duration=8, frequency=5):
...
def rising_bubbles(duration=8, frequency=5):
    ...
    while time.time() - start_time < duration:
        ...
        new_bubbles = []
        for b in bubbles:
            bx = int(b['x'] + fast_sin(b['y'] * 0.5 + b['wobble']) * 2)
            by = int(b['y'])
...
def comet(duration=8, frequency=5):
    ...
    while time.time() - start_time < duration:
        ...
        # Add to trail
        trail.append((cx, cy, hue))
        if len(trail) > 25:
            trail.pop(0)

        # Draw trail
        for i, (tx, ty, th) in enumerate(trail):
            brightness = (i + 1) / len(trail)
...
Examples:
  ...
  python demos.py -e lightning -f 8    # Frequent lightning storms
    ...
    parser.add_argument("--speed", type=float, default=1.0,
                        help="Speed multiplier for animations (default: 1.0)")
        ...
        while args.loops == 0 or loop_count < args.loops:
            for key in effect_keys:
                ...
                if args.duration == 0:
                    ...
                    func(duration=999999, frequency=args.frequency)
                else:
                    func(duration=args.duration, frequency=args.frequency)
